...
def ge_feature(mother_age,mount,preg_week,HC,AC,FL,AMN,BPD,FL_BPD,HC_AC,FL_AC,idx_lat,birth_week):
	lat_feature = np.array([mount[idx_lat],preg_week[idx_lat],HC[idx_lat],AC[idx_lat],FL[idx_lat],AMN[idx_lat],BPD[idx_lat],FL_BPD[idx_lat],HC_AC[idx_lat],FL_AC[idx_lat]], dtype = float)
			...
			if int(birth_weight) > 0:
				if lines > 1:
					...
					#	if count_num_2 == 251 or count_num_2 == 433:
							...
							#print every_file
						X.append(ge_feature(mother_age,mount,preg_week,HC,AC,FL,AMN,BPD,FL_BPD,HC_AC,FL_AC,idx_nonempty[0][sort_idx[-1]],birth_week))
