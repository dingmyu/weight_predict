...
def ge_feature(mother_age,mount,preg_week,HC,AC,FL,AMN,SD,PI,BPD,heart_frequency,FL_BPD,HC_AC,FL_AC,idx_lat, idx_pre ,birth_week):
...
if __name__ == '__main__':#代码重用
				...
				if lines > 1:
					...
					if num_nonempty >= 2:
						...
						if count_num_2 == 251 or count_num_2 == 433:
							...
							print every_file
						X.append(ge_feature(mother_age,mount,preg_week,HC,AC,FL,AMN,SD,PI,BPD,heart_frequency,FL_BPD,HC_AC,FL_AC,idx_nonempty[0][sort_idx[-1]],idx_nonempty[0][sort_idx[-2]],birth_week))
	...
	np.savez(feature_path + 'data.npz',X = X, Y = Y)
